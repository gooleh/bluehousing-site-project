...
const inputClass = (hasError) =>
  `w-full rounded-xl border px-4 py-3 text-ink dark:text-gray-100 bg-white dark:bg-gray-700 placeholder:text-ink-muted/60 dark:placeholder:text-gray-500 transition-colors focus:outline-none focus:ring-2 focus:ring-brand-500/40 focus:border-brand-500 ${
    hasError ? 'border-red-400 bg-red-50/40 dark:bg-red-900/20' : 'border-ink/15 dark:border-gray-600'
  }`;

const Label = ({ htmlFor, ko, en, required }) => (
  <label htmlFor={htmlFor} className="block text-ink dark:text-gray-100 font-semibold mb-2">
    {ko}
    {required && <span className="text-red-400 ml-0.5">*</span>}{' '}
    <span className="text-sm text-ink-muted dark:text-gray-400 font-normal">({en})</span>
  </label>
);
...
const pillInactive = 'bg-white dark:bg-gray-700 text-ink-soft dark:text-gray-300 border-ink/20 dark:border-gray-600 hover:border-brand-400 hover:text-brand-600 dark:hover:border-brand-500 dark:hover:text-brand-400';
  ...
  return (
    <main className="bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="max-w-3xl mx-auto -mt-16 px-4 pb-20 relative z-10">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card p-7 md:p-10">
          <p className="text-ink-soft dark:text-gray-300 mb-2 text-center font-medium">
            아래 양식에 맞춰 정보를 입력해 주시면 빠른 시일 내에 연락드리겠습니다.
          </p>
          <p className="text-ink-muted dark:text-gray-400 mb-8 text-center text-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-7">
            ...
            {/* 현장 사진 안내 */}
            <div className="rounded-xl bg-brand-50 dark:bg-brand-900/20 border border-brand-100 dark:border-brand-800 p-4 flex gap-3">
              <FiCamera className="text-brand-600 text-xl flex-shrink-0 mt-0.5" />
              <div className="text-sm text-ink-soft dark:text-gray-300 leading-relaxed">
                <p className="font-semibold text-ink dark:text-gray-100 mb-1">현장 사진 첨부 안내</p>
