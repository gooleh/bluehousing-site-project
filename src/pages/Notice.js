...
const NoticeSection = ({ title, durationText, additionalInfo }) => (
  <div className="space-y-2">
    <h2 className="text-xl md:text-2xl font-semibold text-ink flex items-center">
      <CheckIcon />
      {title}
    </h2>
    <p className="text-base text-ink-soft leading-relaxed">{durationText}</p>
    {additionalInfo && (
      <p className="text-sm text-ink-muted mt-1">{additionalInfo}</p>
    )}
  </div>
);
